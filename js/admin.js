...
function loadOverview() {
  ...
  db.matches.filter(m => m.status === 'completed').slice(-3).reverse().forEach(m => {
    ...
    activities.push({ icon: 'fas fa-swords', color: '#00b894', text: `Match completed: <strong>${t1 ? esc(t1.name) : 'TBD'}</strong> vs <strong>${t2 ? esc(t2.name) : 'TBD'}</strong>`, time: m.updatedAt });
  });
